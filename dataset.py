    ...
    def __getitem__(self, 
        ...
        return {
            ...
            'target_mask': (((fin_source_tokens != self.pad)
                            ...
                            get_triu_mask(self.max_seq_length))), # (1, max_seq_length, max_seq_length)
            'y': y_hat, # (max_seq_length, ) 
            'source_sent': source_sent,
            'target_sent': target_sent
        }

def get_triu_mask(size):
    """
    ...
    :param size: The size of the mask, which corresponds to the sequence length.
    :type size: int
