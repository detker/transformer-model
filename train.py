...
from dataset import BilingualDataset, get_triu_mask
...
def get_tokenizer(config, 
    ...
    else: tokenizer = Tokenizer.from_file(tokenizer_path)
...
def get_data(config):
    ...
    train_data = BilingualDataset(train_raw_data, source_tokenizer, 
                                  target_tokenizer, config.source_lang, 
                                  config.target_lang, config.sequence_length)
    val_data = BilingualDataset(val_raw_data, source_tokenizer, 
                                target_tokenizer, config.source_lang, 
                                config.target_lang, config.sequence_length)
    test_data = BilingualDataset(test_raw_data, source_tokenizer, 
                                 target_tokenizer, config.source_lang, 
                                 config.target_lang, config.sequence_length)
...
def greedy_search(model, 
    ...
    :param src_mask_test: Source mask for the test set
    :type src_mask_test: torch.Tensor
    :param tgt_tokenizer: Tokenizer for the target language
    :type tgt_tokenizer: Tokenizer
    ...
    while True:
        ...
        tgt_mask_test = get_triu_mask(target_input.shape[1]).to(device)
        dec_out = model.decode(target_input, enc_output, src_mask_test, tgt_mask_test) # (1, S, D)
...
def test_model(config, 
    ...
    for _ in range(config.n_inference):
        ...
        output_tokens = greedy_search(model, enc_output, src_mask_test, target_tokenizer,
                                       config.sequence_length, X_source_test, device)
...
def train_model(config):
    ...
    device = ('cuda' if torch.cuda.is_available() 
                else 'mps' if torch.backends.mps.is_built() or torch.backends.mps.is_available() 
                else 'cpu')
    ...
    model = Transformer.build_transformer(config).to(device)
    ...
    else: print('no model state found. starting from scratch')
    ...
    for epoch in range(init_epoch, config.epochs):
        ...
        for batch in train_data_loader:
            if step % 100 == 0:
                ...
                else: print(f'train_model() took: {(dt_acum/2):.2f} sec.')
                print('-'*40)
                dt_acum = 0
